...
class ForecastController {
    static async createForecastRecords(stationId, forecastData, fileReference) {
        const created = [];
    ...
    static async ensureForecastsForStation(stationId, sourcePath) {
        const existingForecastCount = await prisma_1.prisma.forecastData.count({ where: { stationId } });
        if (existingForecastCount > 0) {
            return [];
        }
        const forecastData = await netcdf_parser_1.NetCDFParser.parseFile(sourcePath || netcdf_parser_1.NetCDFParser.getDefaultFilePath());
    ...
    static async importNetCDF(req, res) {
        try {
            ...
            const sourcePath = file?.path || netcdf_parser_1.NetCDFParser.getDefaultFilePath();
            const forecastData = await netcdf_parser_1.NetCDFParser.parseFile(sourcePath);
            const created = await ForecastController.createForecastRecords(stationId, forecastData, file?.filename || 'wrfout_d01_nc.nc');
