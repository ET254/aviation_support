"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.RunwayMapper = void 0;
class RunwayMapper {
    static map(weather, observation) {
        observation.runwayCondition =
            this.determineRunwayCondition(observation);
        observation.runwayContaminationPercent =
            this.calculateContamination(observation);
        observation.runwayFrictionCoefficient =
            this.calculateFriction(observation);
        observation.brakingAction =
            this.determineBrakingAction(observation.runwayFrictionCoefficient);
        observation.standingWater =
            this.hasStandingWater(observation);
        observation.snowDepth ??= 0;
        observation.slushDepth ??= 0;
        observation.runwayOperational =
            observation.runwayCondition !== "ICE";
    }
    static determineRunwayCondition(wx) {
        if (wx.icing)
            return "ICE";
        if (wx.snowDepth && wx.snowDepth > 0)
            return "SNOW";
        if (wx.slushDepth && wx.slushDepth > 0)
            return "SLUSH";
        if (wx.precipitationType === "RAIN" ||
            wx.precipitationType === "DRIZZLE") {
            return "WET";
        }
        if (wx.relativeHumidity &&
            wx.relativeHumidity >= 90) {
            return "DAMP";
        }
        return "DRY";
    }
    static calculateContamination(wx) {
        switch (wx.runwayCondition) {
            case "DRY":
                return 0;
            case "DAMP":
                return 15;
            case "WET":
                return 35;
            case "SLUSH":
                return 70;
            case "SNOW":
                return 80;
            case "ICE":
                return 100;
            default:
                return 0;
        }
    }
    static calculateFriction(wx) {
        switch (wx.runwayCondition) {
            case "DRY":
                return 0.70;
            case "DAMP":
                return 0.55;
            case "WET":
                return 0.42;
            case "SLUSH":
                return 0.32;
            case "SNOW":
                return 0.28;
            case "ICE":
                return 0.15;
            default:
                return 0.50;
        }
    }
    static determineBrakingAction(friction) {
        if (friction == null)
            return "UNRELIABLE";
        if (friction >= 0.60)
            return "GOOD";
        if (friction >= 0.45)
            return "GOOD_TO_MEDIUM";
        if (friction >= 0.35)
            return "MEDIUM";
        if (friction >= 0.25)
            return "MEDIUM_TO_POOR";
        return "POOR";
    }
    static hasStandingWater(wx) {
        return (wx.precipitationType === "RAIN" &&
            (wx.precipitationIntensity === "HEAVY" ||
                wx.precipitationIntensity === "VIOLENT"));
    }
}
exports.RunwayMapper = RunwayMapper;
//# sourceMappingURL=runway.mapper.js.map