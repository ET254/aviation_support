...
class WeatherService {
    static calculateDerivedParameters(data) {
        const derived = {};
        if (data.temperature !== undefined && data.pressureQnh !== undefined && data.station?.elevation) {
            derived.densityAltitude = this.calculateDensityAltitude(data.temperature, data.pressureQnh, data.station.elevation);
        }
        if (data.windDirection !== undefined && data.windSpeed !== undefined) {
            ...
            derived.crosswindComponent = this.calculateCrosswindComponent(data.windDirection, data.windSpeed, runwayHeading);
            derived.headwindComponent = this.calculateHeadwindComponent(data.windDirection, data.windSpeed, runwayHeading);
            derived.tailwindComponent = this.calculateTailwindComponent(data.windDirection, data.windSpeed, runwayHeading);
        }
    ...
    static calculateDensityAltitude(tempC, pressureQnh, elevation) {
    ...
    static parseRunwayHeading(orientation) {
        if (!orientation)
            return 0;
    ...
    static calculateCrosswindComponent(windDirection, windSpeed, runwayHeading) {
        const angleDiff = windDirection - runwayHeading;
        const rad = (angleDiff * Math.PI) / 180;
        const crosswind = windSpeed * Math.sin(rad);
        return Math.round(crosswind * 10) / 10;
    }
    static calculateHeadwindComponent(windDirection, windSpeed, runwayHeading) {
        const angleDiff = windDirection - runwayHeading;
        const rad = (angleDiff * Math.PI) / 180;
        const headwind = windSpeed * Math.cos(rad);
        return headwind > 0 ? Math.round(headwind * 10) / 10 : 0;
    }
    static calculateTailwindComponent(windDirection, windSpeed, runwayHeading) {
        const angleDiff = windDirection - runwayHeading;
        const rad = (angleDiff * Math.PI) / 180;
        const headwind = windSpeed * Math.cos(rad);
    ...
    static calculateWindChill(tempC, windSpeed) {
    ...
    static async checkThresholds(weatherData) {
        try {
            ...
            if (alerts.length > 0) {
                logger_1.logger.info(`Created ${alerts.length} alerts for weather data ${weatherData.id}`);
            }
        }
        catch (error) {
            logger_1.logger.error('Error checking thresholds:', error);
        }
    }
    ...
    static parseExcelRecord(record) {
        return this.parseCSVRecord(record);
    }
    static analyzeTrends(weatherData) {
        ...
        const sorted = weatherData.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
    ...
    static calculateTrend(values, field) {
        if (values.length < 3)
            return 'stable';
    ...
    static calculateStatistics(weatherData) {
        if (weatherData.length === 0) {
            return {
                count: 0,
                averages: {},
